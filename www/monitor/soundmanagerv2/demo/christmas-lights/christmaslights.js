...
function initChristmasLights() {
  YUI().use("anim", function (Y) {
    function XLSF(oTarget) {
      ...
      var transforms = {
        ie:
          typeof testDiv.style["-ms-transform"] !== "undefined"
            ? "-ms-transform"
            : null,
        moz:
          typeof testDiv.style.MozTransform !== "undefined"
            ? "MozTransform"
            : null,
        opera:
          typeof testDiv.style["OTransform"] !== "undefined"
            ? "OTransform"
            : null,
        webkit:
          typeof testDiv.style.webkitTransform !== "undefined"
            ? "webkitTransform"
            : null,
        prop: null,
      };
      ...
      if (window.location.href.match(/size=/i)) {
        ...
        if (this.lightClass.indexOf("#") !== -1) {
          this.lightClass = this.lightClass.substr(
            0,
            this.lightClass.indexOf("#")
          );
        }
      }
      ...
      function ExplosionFragment(nType, sClass, x, y, vX, vY) {
        ...
        this.burstTween = function () {
          ...
          var phase =
            1 +
            Math.floor(
              Math.min(self.animDuration, this.get("elapsedTime") / 1000) *
                self.burstPhases
            );
      ...
      function Explosion(nType, sClass, x, y) {
        ...
        this.fragments.push(
          new ExplosionFragment(type, sClass, mX, mY, -rnd(scale), -rnd(scale))
        );
        this.fragments.push(
      ...
      function Light(sSizeClass, sClass, nType, x, y, row, col) {
        ...
        this.smash = function (e) {
          ...
          if (!useFollow && !useAngle && transforms.prop) {
            self.o.style[transforms.prop] =
              "rotate(" + Math.random() * plusMinus(20) + "deg)";
          }
        ...
        this.init = function () {
          self.o.className =
            classBase + " " + this.sizeClass + " " + this.sClass;
          self.o.style.left = self.x + "px";
          self.o.style.top = self.y + "px";
      ...
      this.destroyLights = function () {
        ...
        self.startSequence(
          Math.random() > 0.75 ? self.destroyLight : self.destroyRandom,
          33
        );
      };
      ...
      this.destroyLight = function () {
        ...
        if (self.lightSmashCounter < self.lights.length) {
          ...
          var reverseLimit = Math.max(
            0,
            self.lights.length - self.lightSmashCounter - groupSize
          );
...
*/
      ...
      if (
        ...
        document.body.className.match(/home/i)
      ) {
...
*/
      ...
      function mouseOrTouchMove(e) {
        ...
        ((lightCol = Math.floor(
          (x / (self.lightClasses[self.lightClass] * jMax)) * jMax
        )),
          (lightRow = Math.floor(
            (y / (self.lightClasses[self.lightClass] * iMax)) * iMax
          )));
