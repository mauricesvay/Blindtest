...
var Metadata = function (oSound) {
  ...
  this.refresh = function () {
    var offset = 0,
      relWidth = null,
      duration = self.data.givenDuration
        ? self.data.givenDuration
        : oSound.durationEstimate,
      i;
    for (i = 0; i < self.data.length; i++) {
      if (duration) {
  ...
  this.mouseover = function (e) {
    ...
    self.data[this.rel].oNote.style.marginLeft =
      -parseInt(self.data[this.rel].oNote.offsetWidth / 2, 10) + "px";
  ...
  for (i = 0; i < oItems.length; i++) {
    this.data[i] = {
      ...
      startSeconds: self.strToTime(
        oItems[i].getElementsByTagName("span")[0].innerHTML.replace(/[()]/g, "")
      ),
      duration: 0,
      durationMS: null,
      startTimeMS: null,
  ...
  this.data.givenDuration = oDuration.length
    ? self.strToTime(oDuration[0].innerHTML) * 1000
    : 0;
