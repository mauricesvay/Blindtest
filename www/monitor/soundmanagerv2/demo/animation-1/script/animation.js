...
function moveBall(oBall) {
  ...
  var bounce = false;
  if (
    (oBall._vX > 0 && oBall._x + oBall._vX + B_WIDTH > canvasX) ||
    (oBall._vX < 0 && oBall._x + oBall._vX < 0)
  ) {
    oBall._vX *= -1;
    bounce = true;
  }
  if (
    (oBall._vY > 0 && oBall._y + oBall._vY + B_HEIGHT > canvasY) ||
    (oBall._vY < 0 && oBall._y + oBall._vY < 0)
  ) {
...
function mouseMove(e) {
  e = e ? e : event;
  if (
    Math.abs(e.clientX - m_lastX) > M_SPACE ||
    Math.abs(e.clientY - m_lastY) > M_SPACE
  ) {
    createBallAtMouse(e);
    m_lastX = e.clientX;
    m_lastY = e.clientY;
