...
var IS_CHRISTMAS =
  (document.domain.match(/schillmania.com/i) && new Date().getMonth() == 11) ||
...
function resetFilter(o) {
  ...
  var blocks = utils.getElementsByClassName(
    "f-block",
    ["div", "dl"],
    _id("main")
  );
...
function setFilter(e, sFilterPrefix) {
  ...
  var isClear =
    lastSelected && lastSelected == o && utils.hasClass(lastSelected, "active");
  if (oName == "li" && isClear) {
  ...
  if (oName == "li") {
    ...
    var innerText = o.getElementsByTagName("a").length
      ? o.getElementsByTagName("a")[0].innerHTML
      : o.innerHTML; // get inner text (minus link, if one is there)
    sClass =
      sFilterPrefix +
      innerText
        .substr(0, innerText.indexOf("(") != -1 ? innerText.indexOf("(") : 999)
        .toLowerCase()
        .replace(/\s+/i, "-");
...
function fixLinks() {
  ...
  for (var i = l.length; i--; ) {
    ...
    if (
      !s.match(/http/i) &&
      !utils.hasClass(l[i], "norewrite") &&
      (s.match(/doc/i) || s.match(/demo/i) || s.match(/../))
    ) {
      // yep, local.
      ...
      if (!s.match(/\.html/i)) {
        l[i].setAttribute(
          "href",
          s.substr(0, tmp) + "index.html" + s.substr(tmp)
        );
      }
    }
  }
...
if (window.is_home) {
  ...
  function checkBadSafari() {
    ...
    if (
      ...
      // Safari 4 and 5 occasionally fail to load/play HTML5 audio on Snow Leopard due to bug(s) in QuickTime X and/or other underlying frameworks. :/ Known Apple "radar" bug. https://bugs.webkit.org/show_bug.cgi?id=32159
  ...
  soundManager.onready(function () {
    ...
    if (s.useHTML5Audio && s.hasHTML5) {
      ...
      for (item in s.audioFormats) {
        if (s.audioFormats.hasOwnProperty(item)) {
          ...
          items.push(
            '<span class="' +
              (s.html5[item] ? "true" : "false") +
              (!s.html5[item] && needsFlash ? " partial" : "") +
              '" title="' +
              (s.html5[item]
      ...
      li.innerHTML = [
        ...
        soundManager.html5.mp3 || soundManager.html5.mp4
          ? _hasFlash && soundManager.preferFlash
            ...
            : soundManager.html5Only
              ...
              : "&nbsp; (Some flash required; allowing HTML5 to play MP3/MP4, as supported.)" +
                "</p>"
    ...
    var items = utils.getElementsByClassName(
      "muxtape-html5",
      "a",
      _id("inline-playlist")
    );
