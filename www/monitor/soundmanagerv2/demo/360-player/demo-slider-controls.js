...
function Controller(o) {
  ...
  this.cbClick = function (nIndex) {
    document.getElementById("controls").getElementsByTagName("dl")[
      nIndex
    ].className =
      "col" + (this.checked == false || this.checked == "" ? " disabled" : "");
    self.updateExample();
  ...
  this.updateExample = function () {
    if (threeSixtyPlayer) {
      ...
      threeSixtyPlayer.config.waveformDataLineRatio =
        (self.controls[1].value / 100) * 2;

      threeSixtyPlayer.config.waveformDataDownsample = self.controls[2].value;

      threeSixtyPlayer.config.eqDataLineRatio =
        parseInt((self.controls[3].value / 100) * 3 * 1000) / 1000;

      threeSixtyPlayer.config.eqDataDownsample = self.controls[4].value;

      threeSixtyPlayer.config.useEQData = document.getElementById("disabled-1")
        .checked
        ? true
        : false;
      ...
      threeSixtyPlayer.config.useWaveformData = document.getElementById(
        "use-waveform"
      ).checked
        ? true
        : false;

      threeSixtyPlayer.config.waveformDataOutside = document.getElementById(
        "waveform-inside"
      ).checked
        ? false
        : true;

      threeSixtyPlayer.config.eqDataOutside = document.getElementById(
        "eq-inside"
      ).checked
        ? false
        : true;

      threeSixtyPlayer.config.useAmplifier = document.getElementById(
        "use-amplifier"
      ).checked
        ? true
        : false;
    ...
    if (threeSixtyPlayer.lastSound) {
      threeSixtyPlayer.lastSound._360data.circleDiameter =
        self.controls[0].value;

      threeSixtyPlayer.lastSound._360data.circleRadius =
        self.controls[0].value / 2;

      threeSixtyPlayer.lastSound._360data.waveformDataLineRatio =
        (self.controls[1].value / 100) * 2;

      threeSixtyPlayer.lastSound._360data.waveformDataDownsample =
        self.controls[2].value;

      threeSixtyPlayer.lastSound._360data.eqDataLineRatio =
        parseInt((self.controls[3].value / 100) * 3 * 1000) / 1000;

      threeSixtyPlayer.lastSound._360data.eqDataDownsample =
        self.controls[4].value;

      threeSixtyPlayer.lastSound._360data.useEQData = document.getElementById(
        "disabled-1"
      ).checked
        ? true
        : false;
      ...
      threeSixtyPlayer.lastSound._360data.useWaveformData =
        document.getElementById("use-waveform").checked ? true : false;

      threeSixtyPlayer.lastSound._360data.waveformDataOutside =
        document.getElementById("waveform-inside").checked ? false : true;

      threeSixtyPlayer.lastSound._360data.eqDataOutside =
        document.getElementById("eq-inside").checked ? false : true;

      threeSixtyPlayer.lastSound._360data.useAmplifier =
        document.getElementById("use-amplifier").checked ? true : false;
    }
  };
...
\n\
  playNext: <span>" +
      threeSixtyPlayer.config.playNext +
      "</span>,\n\
  autoPlay: <span>" +
      threeSixtyPlayer.config.autoPlay +
      "</span>,\n\
  allowMultiple: <span>" +
      threeSixtyPlayer.config.allowMultiple +
      "</span>,\n\
  loadRingColor: <span>'" +
  ...
  playRingColor: <span>'" +
      ...
      "'</span>,\n\
  backgroundRingColor: <span>'" +
      threeSixtyPlayer.config.backgroundRingColor +
      "'</span>,\n\
  circleDiameter: <span>" +
      threeSixtyPlayer.config.circleDiameter +
      "</span>,\n\
  circleRadius: <span>" +
      threeSixtyPlayer.config.circleRadius +
      "</span>,\n\
  animDuration: <span>" +
      threeSixtyPlayer.config.animDuration +
      "</span>,\n\
  animTransition: <span>Animator.tx.bouncy</span>,\n\
  showHMSTime: <span>" +
      threeSixtyPlayer.config.showHMSTime +
      "</span>,\n\
\n\
  useWaveformData: <span>" +
      threeSixtyPlayer.config.useWaveformData +
      "</span>,\n\
  waveformDataColor: <span>'" +
      threeSixtyPlayer.config.waveformDataColor +
      "'</span>,\n\
  waveformDataDownsample: <span>" +
      threeSixtyPlayer.config.waveformDataDownsample +
      "</span>,\n\
  waveformDataOutside: <span>" +
      threeSixtyPlayer.config.waveformDataOutside +
      "</span>,\n\
  waveformDataConstrain: <span>false</span>,\n\
  waveformDataLineRatio: <span>" +
      threeSixtyPlayer.config.waveformDataLineRatio +
      "</span>,\n\
\n\
  useEQData: <span>" +
      threeSixtyPlayer.config.useEQData +
      "</span>,\n\
  eqDataColor: <span>'" +
      threeSixtyPlayer.config.eqDataColor +
      "'</span>,\n\
  eqDataDownsample: <span>" +
      threeSixtyPlayer.config.eqDataDownsample +
      "</span>,\n\
  eqDataOutside: <span>" +
      threeSixtyPlayer.config.eqDataOutside +
      "</span>,\n\
  eqDataLineRatio: <span>" +
      threeSixtyPlayer.config.eqDataLineRatio +
      "</span>,\n\
\n\
  usePeakData: <span>" +
      threeSixtyPlayer.config.usePeakData +
      "</span>,\n\
  peakDataColor: <span>'" +
      threeSixtyPlayer.config.peakDataColor +
      "'</span>,\n\
  peakDataOutside: <span>" +
      threeSixtyPlayer.config.peakDataOutside +
      "</span>,\n\
  peakDataLineRatio: <span>" +
      threeSixtyPlayer.config.peakDataLineRatio +
      "</span>,\n\
\n\
  useAmplifier: <span>" +
      threeSixtyPlayer.config.useAmplifier +
      "</span>\n\
\n\
}";
  ...
  for (var i = 0; i < items; i++) {
    this.controls[this.controls.length] = new Slider(
      this.o[3 * i + 2].getElementsByTagName("div")[1],
      this.o[3 * i + 1],
      this.o[3 * i + 2].getElementsByTagName("div")[0]
    );
  }
  this.cb = [
...
function Slider(o, oV, oB) {
  ...
  this.barClick = function (e) {
    ...
    self.slide(
      self.x,
      e.clientX - self.o.parentNode.parentNode.offsetLeft - self.o.offsetWidth
    );
  };
...
function demoInit() {
  controller = new Controller(
    document.getElementById("controls").getElementsByTagName("dd")
  );
}
...
function createCP(oInput, oHandler) {
  ...
  if (picker != null) {
    ...
    picker.setValue(
      hex2decArray([c.substr(0, 2), c.substr(2, 2), c.substr(4, 2)]),
      true
    ); // be silent
    return false;
  }
