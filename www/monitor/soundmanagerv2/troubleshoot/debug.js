...
function SM2Debugger() {
  ...
  this.testURL = function (sURL, fOnComplete) {
    ...
    var msg =
      '<a href="' +
      soundManager.url +
      '" title="This should be a valid .SWF URL, not a 404 etc.">' +
      soundManager.url +
      "</a>";
  ...
  this.handleEvent = function (sEventType, bSuccess, sMessage) {
    ...
    if (o) {
      o.className =
        bSuccess == true ? "pass" : bSuccess != false ? bSuccess : "fail"; // true = pass, className as argument, or false == fail
      if (sMessage) {
        var oSpan = o.getElementsByTagName("span")[4];
        if (oSpan) {
          oSpan.innerHTML =
            oSpan.innerHTML + ' <span class="msg">' + sMessage + "</msg>";
        } else {
          o.title = sMessage;
        }
      }
      ...
      if (sEventType == "onload") {
        ...
        if (bSuccess != true) {
          ...
          if (soundManager._overHTTP) {
            document.getElementById("d-flashtojs-offline").style.display =
              "none";
          } else {
            document.getElementById("d-flashtojs-online").style.display =
              "none";
          }
        }
      }
      ...
      soundManager._writeDebug(
        'SM2 debugger warning: Undefined event type "' + sEventType + '"',
        1
      );
    }
  };
    ...
    var flashInfo =
      ...
      (!isSupported
        ? "unsupported (" +
          ...
          ")"
        : flashVersion);
    document.getElementById("d-flashversion").innerHTML =
      "soundManager.flashVersion = " + soundManager.flashVersion + ";";
    if (hasFlash) {
